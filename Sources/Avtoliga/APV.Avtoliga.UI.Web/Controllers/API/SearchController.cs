...
namespace APV.Avtoliga.UI.Web.Controllers.API
{
    public static class SearchController
    {
        ...
        public static void GetSpecialOfferts(out ProductInfo specialOffertOne, out ProductInfo specialOffertTwo)
        {
            specialOffertOne = null;
            specialOffertTwo = null;

            ProductCollection products = ProductManagement.Instance.GetLastSpecialOffers();
            ProductInfo[] productsInfo = products.Transform(false);

            if (productsInfo.Length > 0)
            {
                specialOffertOne = productsInfo[0];
                if (productsInfo.Length > 1)
                {
                    specialOffertTwo = productsInfo[1];
                }
            }
        }
